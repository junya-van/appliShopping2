...
そこで、フローチャートを書いたり色々試行錯誤して以下のようにコードを記述することで無事実装に成功しました。(※商品情報を表すItemBeanクラスはequals()をオーバーライドしておく！また、ItemBeanのフィールドcountのsetterは、this.count += countとして合算するようにした)
